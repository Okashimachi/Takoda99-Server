...
> ⚠ **本戦版は `docs/plan-honsen/plan-h01`・`plan-h02`**。構造化ログ（本plan）に加え、
> 試合中の状態を1画面で俯瞰する**観測ダッシュボード**へ発展させる。本plan（slog）は引き続き有効で、
> h02 の管理者スナップショットと同じ源から出す（イベント=slog / 連続状態=ダッシュボード）。
