...
> ⚠ **訂正（2026-08-05）**: 分担を見直した。~~ワイヤ仕様・クライアント設計は
> `Takoda99-Client-Docs` が正典（server 側の `docs/client-integration.md` は重複解消のため削除済み）。~~
>
> **ワイヤ仕様とタイミングは server 側 `docs/client-integration.md` に戻した。**
> 内容がサーバー実装に完全に従属するため、別リポジトリに置くとサーバーを直したときに
> 更新されず必ず腐るのが理由。`Takoda99-Client-Docs` は**クライアント側の設計**
> （アーキテクチャ・状態管理・ディスパッチ層・打鍵判定・画面遷移）を担う。

| | 担当 |
|---|---|
| `docs/client-integration.md`（server） | ワイヤ仕様とタイミング。**どの型がいつ届くか** |
| `Takoda99-Proto` | メッセージの型（正典） |
| `Takoda99-Client-Docs` | クライアント側の設計 |

結合時に食い違いが見つかったら**ドキュメントではなくサーバーの実挙動を正**として直す。
...
- 表示名は最大6文字（ルーン単位）・制御文字は除去される（`matchmaking.SanitizeDisplayName`）
