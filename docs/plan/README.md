...
- タスクの進捗・担当は **GitHub issue** が正典（`gh issue list`）
...
| plan | 内容 | issue |
|---|---|---|
| [13](plan-13_ヘッドレスsimとWeb結合.md) | **ヘッドレスsim＋Web結合**。1試合を通しで遊べる状態にする | #12 |
| [14](plan-14_決着保証テスト.md) | **決着保証テスト**。試合が必ず終わることを固定する | #34 |
| [15](plan-15_soloモード起動構成.md) | **solo モード起動構成**。WebFront 開発のブロックを外す | #36 |
| [16](plan-16_本番前のmatchモード復帰.md) | 本番前に match へ戻す（**plan-15 の方式B なら不要**） | #37 |
...
```
plan-13 (sim+Web結合) ─┬─→ plan-14 (決着保証)
                       └─→ plan-18 (負荷テスト)
plan-15 (solo構成) ────┬─→ plan-13 の Web 結合
                       ├─→ plan-16 (不要化)
                       └─→ plan-21 (WebGL疎通) ─→ plan-22 (Unity結合)
...
| plan | 判定 |
|---|---|
| [16](plan-16_本番前のmatchモード復帰.md) | plan-15 で方式B を採ったなら**不要**（issue を close） |
