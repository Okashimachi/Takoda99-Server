...
[接続]
  ...
  │  ├─← ForcedEliminationWarning ← 毎周期・**全店へ（脱落済みも含む）**
...
| `ForcedEliminationWarning` | **全店（脱落済みも含む）** | 足切り予告 | 1周期に1回 |
...
```json
{ "customerId": "c-42", "attribute": "Normal", "orderCount": 8,
  "words": ["たこやき", "おおきに", "そーすたっぷりやで", "あつあつ",
            "まいどあり", "かつおぶし", "ふなもりで", "またきてや"],
...
**★`orderCount` は属性とは無関係に抽選され、当日も config で変わる。固定値を前提にしないこと。**
現在の既定は **2個 / 4個 / 8個** の3段階（出現率 35% / 35% / 30%・平均4.5個）で、
**同じ属性の客にどの個数も出る**（h36 以前は属性ごとに固定で Normal 3 / Bonus 3 / Claimer 2 / Buzz 6 だった）。
見た目（属性）と量（個数）は独立なので、**キャラの絵から個数を推測しないこと**。
**1語は最長でも約48打鍵**に収まる（h30 以前は最大85打鍵の語があった）。難度は
「1語の長さ」ではなく「何語打つか」で上がる設計なので、**お題表示は複数語の連続を前提に組む**
（**最大8語が一度に届く**ので、レイアウトは8語で崩れないこと）。
...
次の足切りの予告。**全店（脱落済みを含む）へ常時届く**（予選は生存店・予告時のみだった）。
右パネルが常設UIなので、「次の足切りまであと何秒」「誰が切られるか」が常に届いている。

> 🔴 **脱落後（観戦中）も届き続ける**（2026-08-18 変更）。
> 99人中98人は脱落してから試合終了までを観戦者として過ごすため、ここが止まると
> 秒読みが最後の値で凍り、「順位は動いているのに残り0秒のまま、脱落予定の顔ぶれも古い」
> という矛盾が画面に残る。次に誰が切られるかは観戦の見どころそのものなので配り続ける。
>
> - **`selfAtRisk` は脱落済みの店では必ず `false`**（脱落した店は「脱落圏内」になりえない）
> - **中身は宛先で変えない**。`cutStoreIds` など他のフィールドは生存店へ送るものと同一
> - 全ステージ消化後・全滅後は送られない（秒読みの対象が無いため）
