...
plan-h30 で **1語を短くし（level 17 で 85打鍵 → 43打鍵前後）、注文数を上げた**。
さらに plan-h36 で注文数が**属性から切り離され**、`customer.orderTiers`（2個/4個/8個・
重み 35/35/30・平均4.5個）の抽選になった。当日これが合わないと感じたら、**上から順に**試す。

1. **注文数を軽くする（ビルド不要・数秒・これが一番効く）**

   運営UI（takoda99-config）で `customer.orderTiers` を触る。h36 以前（平均3.05個）の
   体感に寄せるなら **2個/3個/6個・重み 10/85/5**。単に軽くしたいだけなら
   重い段（8個）の重みを下げる（例 35/45/20）。
   1語が短いままでも、1客あたりの打鍵量と加点はこれで下がる。**まずこれで足りるか見る。**

   > 🔴 **最も軽い段（2個）を消さないこと。** 全段を重くすると「打ち切れないまま終わる」客が
   > 増え、遅い人が0点のまま終わる（実力相関 0.95 → 0.88）。h36 §0.3 の実測。
...
| お題が短すぎる / 注文が多すぎる | `customer.orderTiers` | 重い段(8個)の重みを下げる。h36 以前へ寄せるなら 2/3/6・10/85/5（§2 ②'）。🔴 **2個の段は消さない** |
