	...
	cases := []struct {
		...
		{"MatchStart",
			MatchStart{
				...
				Params: GameParametersPublicSubset{InitialLife: 3, MaxStores: 99, StormThresholdPct: 0.1, FinalStageAliveThreshold: 20, FinalRushAliveThreshold: 10},
				Phase:  PhaseEarly,
				Stores: []StoreSummary{{StoreId: "s1", DisplayName: "s1", EvalNormalized: 0, Rank: 0, CreditLife: 3, Alive: true}},
			},
			`{"matchId":"m-1","selfStoreId":"s1","params":{"initialLife":3,"maxStores":99,"stormThresholdPct":0.1,"finalStageAliveThreshold":20,"finalRushAliveThreshold":10},"phase":"Early","stores":[{"storeId":"s1","displayName":"s1","evalNormalized":0,"rank":0,"creditLife":3,"alive":true}]}`},
		{"CustomerArrived",
